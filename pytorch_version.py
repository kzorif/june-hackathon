...
# =================================================================================
#
# UTILITY FUNCTIONS (FROM ULTRALYTICS/UTILS/OPS.PY AND ORIGINAL SCRIPT)
#
# =================================================================================
...
# THIS IS THE OFFICIAL, CORRECTED NON_MAX_SUPPRESSION FUNCTION
def non_max_suppression(
    ...
    Args:
        prediction (torch.Tensor): A tensor of shape (batch_size, 4 + num_classes, num_proposals).
        conf_thres (float): The confidence threshold for filtering boxes.
        iou_thres (float): The IoU threshold for NMS.
        classes (list[int], optional): A list of class indices to consider.
        agnostic (bool): If True, perform class-agnostic NMS.
        multi_label (bool): If True, each box can have multiple labels.
        max_det (int): The maximum number of detections to keep.

    Returns:
        (list of torch.Tensor): A list of tensors, where each tensor contains the final detections
                                for an image in the batch in the format [x1, y1, x2, y2, conf, class_id].
    """
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        x = x.transpose(0, -1)  # [num_proposals, 4+nc]
...
# =================================================================================
#
# PURE PYTORCH YOLOV8 IMPLEMENTATION (INDEPENDENT OF ULTRALYTICS)
#
# =================================================================================
...
def load_yolo_model_from_pt(weights_path, device):
    ...
    )  # Change to strict=True for confidence
    ...
    model.to(device)
    model.eval()
    print("Model loaded successfully into a pure PyTorch object.")
    return model
