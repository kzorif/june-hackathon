...
class DetectionModelMAX(nn.Module):
    ...
    def __call__(self, x: driver.Tensor):
        ...
        processed_output = self.forward_postprocess(raw_outputs_np)
        return raw_outputs_np, processed_output
...
def load_yolo_model_from_pt(weights_path):
    print(f"Loading weights from {weights_path} into MAX model shell...")
    if not _torch_available:
    ...
    print("Weights loaded and fused.")
    return max_model
...
if __name__ == "__main__":
    ...
    _, processed_output = max_model(max_tensor)

    final_detections = non_max_suppression_max(
